...
export async function GET(
  ...
  try {
    ...
    const days = await prisma.k12Day.findMany({
      where: {
        week: { classId },
    ...
    console.error('Failed to fetch K12 days:', error);
...
export async function POST(
  ...
  try {
    ...
    const day = await prisma.k12Day.create({
      data: {
        weekId: week.id,
        date: dateObj,
        dayOfWeek,
    ...
    console.error('Failed to create K12 day:', error);
